...
void create_output_set_UI(Equation* equation) {
	...
	switch (choice) {
	case 1:
		std::cout << "Enter the start range for number set: "; std::cin >> start;
		std::cout << "Enter the end range for number set: "; std::cin >> end;
		equation->change_output_set(equation->create_output_set(start, end));
		equation->print_output_set(); break;
	case 2:
		return;
	default:
...
int main() {
	...
	while (!quit) {
		...
		std::cout << "Enter number choice: "; std::cin >> choice;
		switch (choice) {
		case 1:
			generate_equation(); std::cout << std::endl; break;
		case 2:
			generate_output(); std::cout << std::endl; break;
		case 3:
			generate_and_solve(); std::cout << std::endl; break;
		case 4:
			quit = true; break;
		default:
			std::cout << "Please enter a number between 1 and 4!" << std::endl; std::cout << std::endl; break;
		}
	}
	return 0;
}
